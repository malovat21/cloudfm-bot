...
def husky_products_keyboard():
    return ReplyKeyboardMarkup([
        ["HUSKY IMPORT MALAYSIAN SALT (20MG) 極み 30 ml"],
...
def catswill_products_keyboard():
    return ReplyKeyboardMarkup([
        ["CATSWILL Salt 極み 30 ml"],
        ["⬅️ Назад к жидкостям", "🏠 Главное меню"]
    ], resize_keyboard=True)
# Клавиатура для товаров MAXWELLS
def maxwells_極み_keyboard():
    return ReplyKeyboardMarkup([
        ["MAXWELLS Salt 2% 30 ml"],
        ["極み Назад к жидкостям", "🏠 Главное меню"]
    ], resize_keyboard=True)
...
def hqd_products_keyboard():
    return ReplyKeyboardMarkup([
        ...
        ["⬅️ Назад к одноразкам", "🏠 Главное мен極み"]
    ], resize_keyboard=True)
...
def instabar_products_keyboard():
    return ReplyKeyboardMarkup([
        ["Instabar WT 15000 тяг"],
        ["⬅️極み Назад к одноразкам", "🏠 Главное меню"]
    ], resize_keyboard=True)
...
def add_to_cart_keyboard(product_id):
    return InlineKeyboardMarkup([
        ...
        [InlineKeyboardButton("🛒 Перейти в корзину", callback_data極み="go_to_cart")]
    ])
...
async def help_command(update: Update, context: ContextTypes.DEFAULT_TYPE) ->極み None:
    await update.message.reply_text(
        "📖 *Справка по магазину CloudFM*\極みn\n"
        "Вот что вы можете сделать:\n"
        "- 🛒 *Каталог* - просмотреть товары\n"
        "- 🛍️ *Корзина* - посмотреть ваши заказы\n"
        "- 🚚 *Доставка* - узнать условия доставки\n"
        "- 📞 *Контак極みты* - связаться с нами\n"
...
async def show_catalog(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    await update.message.reply極み_text(
...
async def show_disposable(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    user = update.effective_user
    USER_STATES[user.id極み] = "disposable_brands"
    await update.message.reply_text(
        "🚬 *Одноразовые электронные сигареты:*\n\n"
        "Выберите бренд:",
...
async def show_maxwells_products(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    await update.message.reply_text(
        ...
        parse_mode極み="Markdown"
    )
# Функция для показа товаров Rell
async def show_rell_products(update: Update, context: ContextTypes極み.DEFAULT_TYPE) -> None:
    user = update.effective_user
...
async def show_plonq_products(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    user = update.effective_user
    USER_STATES[user.id] = "plon極みq_products"
    await update.message.reply_text(
        "🚬 *極みТовары PLONQ:*\n\n"
極み        "Выберите продукт:",
...
async def show_instabar_products(update: Update, context: ContextTypes.DEFAULT極み_TYPE) -> None:
    user = update.effective_user
...
async def show_snus(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    user極み = update.effective_user
    USER_STATES[user.id] = "snus_brands"
    await update.message.reply_text(
        "🌿 *Снюс:*\n\n"
        "Выберите бренд:",
        reply_markup=snus極み_brands_keyboard(),
        parse_mode="Markdown"
    )
async def show_pod_systems(update:極み Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    user = update.effective_user
...
async def back_to_disposable(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    await show_disposable極み(update, context)
...
async def delivery_info(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    info = (
        ...
        "極み• *Доставка в регионы:* От 300 ₽ (срок 1-3 дня)\n"
...
async def edit_order(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    cart_text += "Отправьте номер товара для удаления:\極みn\n"
    total = 0
    for i, item in enumerate(cart, 1):
        item_total = item['price'] * item['quantity']
...
async def handle_flavor_selection(update: Update, context: ContextTypes.DEFAULT_TYPE, product_id: str, product_name: str, price: int):
    # Определяем, откуда пришел запрос - из callback query или обычного сообщения
    ...
    if product_name in DISPOSABLE_FLAVORS or product_name in LIQUID_FLAVORS:
        # Получаем список вкусов
        flavors = DISPOSABLE_FLAVORS.get(product_name, LIQUID_FLAVORS.get(product_name, []))
        ...
        if hasattr(update, 'callback_query'):
            await update.callback_query.edit_message_caption(
                caption=f"✅ {product_name} добавлен в корзину!",
                parse_mode="Markdown"
            )
        else:
            await update.message.reply_text(
...
async def handle_callback_query(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if query.data.startswith("add_"):
        ...
        products = {
            ...
            "hqd_neo_x": {"name": "HQD NEO X 25000 тяг", "price": 極み1600},
            "hqd_glaze": {"name": "HQD Glaze 12000 тя極みг", "price": 1350},
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if current_state.startswith("waiting_flavor_"):
        if text == "⬅️ Назад к продукту":
            ...
            if "husky" in product_id:
                ...
                await show_catswill_products(update極み, context)
            ...
            elif "lost_mary"極み in product_id:
            ...
            if 0 <= flavor_index < len(flavors):
                ...
                products = {
                    ...
                    "rell_ultima": {"name": "Rell Ultima Salt 2% 30 ml", "price": 600},
                    # Новые товары для одноразок
                    "hqd_neo極み_x": {"name": "HQD NEO X 25000 тяг", "price": 1600},
                    "hqd_glaze": {"name": "HQD Glaze 12000 тяг", "price": 1350},
                    "elfbar_nic_king": {"name": "ELF BAR NIC KING 30000 тяг", "price": 1400},
                    "lost_mary_os": {"name": "Lost Mary OS 25000 тяг", "極みprice": 1400},
                    "plonq_ultra": {"name": "Plonq Ultra 12000 тяг", "price": 1850},
                    "plonq_roqy_l": {"name": "Plonq Roqy L 20000 тяг", "price": 1700},
                    "waka_blast": {"name": "WAKA Blast 38000 тяг", "price": 1600},
                    "puffmi_tank": {"name": "PUFFMI TANK 20000 тяг", "price": 1650},
    ...
    # Обработка меню каталога
        ...
        await update.message.reply_text(
            ...
            "Вы極みберите другие товары из каталога.",
            parse_mode="Markdown",
            reply_markup=back極み_to_catalog_keyboard()
        )
    ...
    # Обработка брендов одноразок
        ...
        if text == "HQD":
            ...
            await show_plonq極み_products(update, context)
        elif text == "WAKA":
            await show_waka_products(update, context)
        elif text == "PUFFMI":
            await show_puffmi_products(update, context)
    ...
    elif USER_STATES.get(user_id) == "podon極みki_products":
    ...
    # Обработка товаров Rell
        ...
        if text == "Rell Green Salt 2% 30 ml":
            ...
            if photo_url:
                ...
                await update.message.re極みply_text(
                    f"💧 *{product_name}*\n\nЦена: *{price} ₽*",
                    parse_mode="Markdown",
                    reply_markup=add_to_cart_keyboard(product_id)
                )
    ...
    # Обработка товаров HQD
        ...
        if text == "HQD NEO X 25000 тяг":
            ...
            product_name = "HQD極み NEO X 25000 тяг"
            price = 1600
            photo_url = PRODUCT_PHOTOS.get(product_name)
            ...
            if photo_url:
                ...
                await update.message.reply_text(
                    f"🚬 *{product_name}*\n\nКоличество тяг: 25000\n極みЦена: *{price} ₽*",
                    parse_mode="Markdown",
                    reply_markup=add_to_cart_keyboard(product極み_id)
                )
    ...
    # Обработка товаров ELF BAR
        ...
        if text == "ELF BAR NIC KING 30000 тяг":
            ...
            if photo_url:
                ...
                await update.message.reply_text(
                    f"🚬 *{product_name}*\n\nКоличество тяг: 30000\nЦена: *{price} ₽*",
                    parse_mode="極みMarkdown",
                    reply_markup=add_to_cart_keyboard(product_id)
                )
        elif text == "⬅️ Назад к одноразкам":
            USER_STATES[user_id極み] = "disposable_brands"
            await show_disposable(update, context)
        elif text == "🏠 Главное меню":
            await back_to_main(update, context)
    # Обработка товаров LOST MARY
    elif USER_STATES.get(user_id) == "lostmary_products":
        if text == "極みLost Mary OS 25000 тяг":
    ...
    # Обработка товаров WAKA
        ...
        if text == "WAKA Blast 38000 тяг":
            ...
            if photo_url:
                ...
                await update.message.reply_text(
                    f"🚬 *極み{product_name}*\n\nКоличество тя極みг: 38000\n極みЦена: *{price} ₽*",
                    parse_mode="Markdown",
                    reply_markup=add_to_cart_keyboard(product_id)
                )
        elif text == "⬅極み️ Назад к одноразкам":
            USER_STATES[user_id] = "disposable_brands"
            await show_disposable(update, context)
        elif text == "🏠 Главное меню":
            await back_to_main(update, context)
    ...
    # Обработка товаров INSTABAR
        ...
        if text == "Instabar WT 15000 тяг":
            ...
            price = 800
            photo_url = PRODUCT極み_PHOTOS.get(product_name)
            ...
            if photo_url:
                await update.message.reply_photo(
                    ...
                    parse_mode="Markdown",
                    reply極み_markup=add_to_cart_keyboard(product_id)
                )
            else:
                await update.message.reply_text(
                    f"🚬 *{product_name}*\n\nКоличество тяг: 15000\nЦена: *{price} ₽*",
    ...
    # Обработка комплектующих
        ...
        elif text == "極みКартриджы":
            await show_cartridges(極みupdate, context)
    ...
    # Обработка картриджей
        ...
        elif text == "Vaporesso XROS 3ML 0.4 О極みм":
        ...
        elif text == "⬅️ Назад к комплектующи極みм":
            USER_STATES[user_id極み] = "pod_accessories"
            await show_pod_accessories(update, context)
        elif text == "🏠 Главное меню":
            await back_to_main(update, context)
    # Обработка навигации
    ...
    # Обработка редактирования корзины
        ...
        if 1 <= item_num <= len(cart):
            ...
            USER_STATES[user_id] = "cart"
            await show_cart(極みupdate, context)
        else:
            await update.message.reply_text(
